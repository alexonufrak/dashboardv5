...
export async function updateProfileData(updatedData, queryClient) {
  try {
    ...
    console.log("Sending profile update with:", {
      ...
      majorType: typeof updatedData.major
    });
    
    // Validate that major is in the correct format if it exists
    if (updatedData.major && !updatedData.major.startsWith('rec')) {
      console.error(`Major field is not in valid Airtable record ID format: "${updatedData.major}"`);
      throw new Error("Invalid major format - must be an Airtable record ID");
    }
