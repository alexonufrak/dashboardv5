...
export function useTeamSubmissions(teamId, milestoneId) {
  ...
  return useQuery({
    ...
    // Cache settings
    staleTime: 60 * 60 * 1000, // 1 hour - match the API cache setting
    cacheTime: 24 * 60 * 60 * 1000, // 24 hours - keep data in cache longer to persist between page navigations
    enabled: !!teamId,
  });
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const url = initiativeName 
        ? `/api/user/check-initiative-conflicts?contactId=${contactId}&initiative=${encodeURIComponent(initiativeName)}&_t=${timestamp}`
        : `/api/user/check-initiative-conflicts?contactId=${contactId}&_t=${timestamp}`
