...
export function useTeamSubmissions(teamId, milestoneId) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        // Check if team data contains direct submissions property we can use
        // This would improve performance by avoiding repeated API calls if the data is already available
        ...
        if (teamData?.submissions?.length > 0) {
          ...
          // This is a fallback in case the team data contains submission data directly
          ...
          if (relevantSubmissions.length > 0) {
            ...
            // Return early with the submissions from team data
            return {
        ...
        // If no cached data, check if we can extract any submission data from team data
        const teamData = queryClient.getQueryData(['teams'])?.find(team => team.id === teamId);
        if (teamData?.submissions?.length > 0) {
          const relevantSubmissions = teamData.submissions.filter(submission => {
            return (
        ...
        // Re-throw to trigger error handling in React Query if no fallback data found
        throw err;
      }
    },
