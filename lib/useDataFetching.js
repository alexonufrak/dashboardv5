 ...
 * This implementation uses a direct approach that leverages the team and milestone cache
 */
export function useTeamSubmissions(teamId, milestoneId) {
  const queryClient = useQueryClient();

  // Return a pre-configured query that leverages cached data
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Get team data from cache first
      const teamData = queryClient.getQueryData(['teams'])?.find(team => team.id === teamId);
      
      // Check if team has direct submissions field
      if (teamData?.fields?.Submissions && Array.isArray(teamData.fields.Submissions)) {
        // Format submissions to expected structure and filter for this milestone
      ...
      // If we don't have submissions in the team record, simply return empty array
      return { submissions: [] };
    },
    // Keep cache for longer periods since we're not making external API calls
    staleTime: 10 * 60 * 1000, // 10 minutes
    cacheTime: 30 * 60 * 1000,  // 30 minutes
    enabled: !!teamId && !!milestoneId,
  });
}
