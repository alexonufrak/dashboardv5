...
export function useTeamSubmissions(teamId, milestoneId) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!teamId) {
        console.log("Missing teamId - returning empty submissions array");
        return { submissions: [] };
      }
      
      console.log(`Fetching submissions for teamId=${teamId}, milestoneId=${milestoneId || 'all'}`);
      ...
      const data = await response.json();
      console.log(`Received ${data.submissions?.length || 0} submissions`);
