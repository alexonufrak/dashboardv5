...
export function useTeamSubmissions(teamId, milestoneId) {
  ...
  return useQuery({
    ...
    staleTime: 5 * 60 * 1000, // 5 minutes - increase to reduce API calls
    cacheTime: 30 * 60 * 1000, // Keep cached data for 30 minutes even after unmounting
    enabled: !!teamId && !!milestoneId, // Only run query if both IDs are available
    retry: 1, // Reduce to just one retry to avoid overwhelming API
