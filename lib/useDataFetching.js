...
export function useUpdateProfile() {
  ...
  return useMutation({
    mutationFn: async (updatedData) => {
      ...
      console.log(`[${transactionId}] Preparing fetch to: ${apiUrl}`);
      console.log(`[${transactionId}] Method: PATCH`); // Assuming it should always be PATCH here
      console.log(`[${transactionId}] Headers being sent:`, JSON.stringify(authHeaders));
      // *** Explicitly log the credentials option being passed ***
      const fetchOptions = {
        method: "PATCH", // Or POST if the override is intentional, but PATCH is better
        headers: {
        ...
        credentials: 'include', // Hardcode 'include' here for testing if needed
        cache: 'no-store', // Prevent caching of auth requests
        mode: 'cors',
        body: JSON.stringify(modifiedData),
        signal: AbortSignal.timeout(10000)
      };
      console.log(`[${transactionId}] Fetch options:`, JSON.stringify(fetchOptions));
      console.log(`[${transactionId}] Current document.cookie before fetch:`, document.cookie); // Check cookie state right before

      // Use the logged options
