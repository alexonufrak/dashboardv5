...
export function useTeamSubmissions(teamId, milestoneId) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Per the schema, check if team has direct Submissions field
      if (teamData.fields?.Submissions && Array.isArray(teamData.fields.Submissions)) {
        trace.addStep("Check team Submissions field", 
                     `Found ${teamData.fields.Submissions.length} direct submission IDs`);
        
        // Get milestone-specific submissions from the Submissions table if available
        if (milestoneId) {
          try {
            // Make a direct API call to check if this submission is linked to our milestone
            // This is more reliable than guessing based just on the team's submissions list
            trace.addStep("API call preparation", `Calling /api/teams/${teamId}/submissions?milestoneId=${milestoneId}`);
            
            const response = await fetch(`/api/teams/${teamId}/submissions?milestoneId=${milestoneId}`);
            
            if (response.ok) {
              const data = await response.json();
              if (data.submissions && data.submissions.length > 0) {
                trace.addStep("API call result", `Found ${data.submissions.length} submissions for milestone ${milestoneId}`);
                // Include trace data for debugging
                return { ...data, _trace: trace.steps };
              } else {
                trace.addStep("API call result", "No submissions found for this milestone");
              }
            } else {
        ...
        // Client component will handle matching the correct milestone
        trace.addStep("Fallback approach", "Formatting submission IDs from team data");
        ...
        const submissions = teamData.fields.Submissions
          ...
          .map(submissionId => ({
            id: submissionId,
            teamId: teamId,
            // When we don't know which milestone it's for, include milestone reference if available
        ...
        trace.addStep("Check team Submissions field", "Not found or empty");
      }
