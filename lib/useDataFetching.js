...
export function useTeamSubmissions(teamId, milestoneId) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // If milestone ID is missing, we can't fetch properly
      if (!milestoneId) return { submissions: [] }
      ...
      try {
        ...
        console.error(`Error fetching submissions for milestone ${milestoneId}:`, err)
        // Re-throw to trigger error handling in React Query
        throw err
      }
    },
