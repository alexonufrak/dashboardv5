...
/**
 * Custom hook for team submissions data fetching with caching
 */
export function useTeamSubmissions(teamId, milestoneId) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let url = `/api/teams/${teamId}/submissions`
      if (milestoneId) {
    ...
    staleTime: 2 * 60 * 1000, // 2 minutes - shorter time since submissions may change frequently
    enabled: !!teamId,
    retry: 2
  })
}
...
export async function updateProfileData(updatedData, queryClient) {
  try {
    ...
    // Log the data being sent, focusing on the major field
    console.log("Profile update data:", {
      firstName: dataToSend.firstName,
      lastName: dataToSend.lastName,
      major: dataToSend.major,
      majorType: typeof dataToSend.major
    });
    
    // If major exists but doesn't look like an Airtable ID, try to fix it
    if (dataToSend.major && typeof dataToSend.major === 'string' && !dataToSend.major.startsWith('rec')) {
      console.warn(`Major field has invalid format: "${dataToSend.major}"`);
      
      // Clear the invalid major field to prevent API errors - the API will just skip updating it
      console.log("Removing invalid major field from update request");
      delete dataToSend.major;
      
      // We'll continue with the request but without the major field
      toast.warning("Major field format was invalid and will not be updated");
    }
