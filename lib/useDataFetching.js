...
export function useUpdateProfile() {
  ...
  return useMutation({
    mutationFn: async (updatedData) => {
      ...
      // Try to get session token from sessionStorage as fallback
      let authHeader = {};
      if (typeof window !== 'undefined') {
        try {
          // First try the dedicated token we stored in callback.js
          const storedToken = sessionStorage.getItem('auth0.id_token');
          
          if (storedToken) {
            console.log('Found id_token in sessionStorage, using as Bearer token');
            authHeader = {
              "Authorization": `Bearer ${storedToken}`
            };
          } else {
            // Try to parse from session data as backup
            const sessionData = JSON.parse(sessionStorage.getItem('auth0.session') || '{}');
            if (sessionData.id_token) {
              console.log('Found id_token in auth0.session, using as Bearer token');
              authHeader = {
          ...
          console.warn('Could not get auth token from sessionStorage:', e);
        }
      }
