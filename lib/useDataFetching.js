...
export function useTeamSubmissions(teamId, milestoneId) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const cacheBreaker = `${url}${url.includes('?') ? '&' : '?'}_t=${Date.now()}`;
      
      // Make direct API call to get filtered submissions
      const response = await fetch(cacheBreaker);
      ...
      if (!milestoneId && data.submissions && data.submissions.length > 0) {
        ...
        Object.keys(submissionsByMilestone).forEach(mId => {
          queryClient.setQueryData(['submissions', teamId, mId], {
            submissions: submissionsByMilestone[mId]
          });
        });
      }
    ...
    // Cache settings - reduced stale time to ensure more frequent refreshes
    staleTime: 5 * 60 * 1000, // 5 minutes
    cacheTime: 60 * 60 * 1000, // 1 hour - keep data in cache longer to persist between page navigations 
    enabled: !!teamId,
