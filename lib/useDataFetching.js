  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        // Always make a network request for fresh data
        console.log(`Making network request for submissions: ${url}`);
        const response = await fetch(url);
        
        // Log complete response details for debugging
        console.log(`Submission API response status: ${response.status} ${response.statusText}`);
        ...
        if (!response.ok) {
          ...
          try {
            ...
            console.error(`Submission fetch error (${response.status}): ${errorText}`);
          } catch (textError) {
            console.error('Could not extract error text from response');
          }
          
          throw new Error(`Failed to fetch submissions: ${response.statusText}. ${errorDetails}`);
        }
        ...
        try {
          ...
          console.log(`Raw submission API response (first 200 chars): ${responseText.substring(0, 200)}...`);
        ...
        // Log submission data details for debugging
        console.log(`Received ${submissions.length} submissions for milestone ${milestoneId}`);
        if (submissions.length > 0) {
          console.log(`First submission ID: ${submissions[0].id}, created: ${submissions[0].createdTime}`);
        }
        ...
        // Re-throw to trigger error handling in React Query if no cached data
        throw err;
      }
    },
    staleTime: 60 * 1000, // 1 minute - balance between freshness and performance
    cacheTime: 10 * 60 * 1000, // Keep cached data for 10 minutes even after unmounting
