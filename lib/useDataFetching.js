...
export function useProgramData() {
  return useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      try {
        // Remove cache-breaking timestamp to leverage server caching
        const response = await fetch('/api/user/participation', { signal });
 ...
 * Improved implementation with better cache handling for consistent UI updates
 */
export function useTeamSubmissions(teamId, milestoneId) {
  const queryClient = useQueryClient();
  const userId = typeof window !== 'undefined' ? window._userId : null;
  
  // Return a query optimized for the server's caching and rate limiting strategies
  return useQuery({
    // Include user ID in the query key for cache isolation
    queryKey: userId ? ['submissions', userId, teamId, milestoneId] : ['submissions', teamId, milestoneId],
    queryFn: async ({ signal }) => {
      ...
      try {
        // Make API call with AbortController signal for cancellation
        // Let server cache headers control caching instead of client-side cache busting
        ...
        if (response.status === 429) {
          ...
          const cacheKey = userId ? ['submissions', userId, teamId, milestoneId] : ['submissions', teamId, milestoneId];
          const cachedData = queryClient.getQueryData(cacheKey);
          if (cachedData) {
            console.log('Returning stale cached data while rate limited');
            return cachedData;
          }
        ...
        if (!milestoneId && data.submissions && data.submissions.length > 0) {
          ...
          Object.entries(submissionsByMilestone).forEach(([mId, submissions]) => {
            const cacheKey = userId 
              ? ['submissions', userId, teamId, mId] 
              : ['submissions', teamId, mId];
        ...
        // If the request was aborted, don't log as error
        if (error.name === 'AbortError') {
        ...
        // Return any cached data if available rather than failing
        const cacheKey = userId ? ['submissions', userId, teamId, milestoneId] : ['submissions', teamId, milestoneId];
        const cachedData = queryClient.getQueryData(cacheKey);
        if (cachedData) {
    ...
    // Optimized cache settings aligned with server-side caching strategy
    staleTime: 5 * 60 * 1000, // 5 minutes before background refresh
    gcTime: 60 * 60 * 1000, // 1 hour garbage collection time (previously cacheTime)
    enabled: !!teamId,
