...
export function useUpdateProfile() {
  ...
  return useMutation({
    // Step 1: Define the mutation function that handles the API call
    mutationFn: async (updatedData) => {
      // Create a transaction ID for this update (useful for logging/debugging)
      const transactionId = `profile-update-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
      console.log(`[${transactionId}] Starting profile update transaction`);
      ...
      console.log(`[${transactionId}] Data prepared for API:`, {
        firstName: dataToSend.firstName,
        lastName: dataToSend.lastName,
        hasMajor: dataToSend.major !== null,
        majorValue: dataToSend.major
      });
      
      // Create auth header for our alternative auth approach
      // This bypasses issues with cookies not being sent with PATCH requests
      const authHeader = btoa(JSON.stringify({
        contactId: updatedData.contactId,
        timestamp: Date.now() // Prevent replay attacks
      }));
      
      // Make the API request with our alternative auth approach
      // This adds auth info in both query params and headers
      const response = await fetch(`/api/user/profile?auth=${encodeURIComponent(authHeader)}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          "X-Transaction-ID": transactionId,
          "X-Auth-Verification": authHeader
        },
        credentials: 'include', // Still include cookies as a fallback
        body: JSON.stringify(dataToSend),
        // Use AbortController with timeout for better error handling
      ...
      if (!response.ok) {
        ...
        console.error(`[${transactionId}] API error:`, errorMessage);
      ...
      try {
        ...
        console.error(`[${transactionId}] Error parsing response:`, e);
      ...
      console.log(`[${transactionId}] Profile update successful:`, {
      ...
      if (!profileData || typeof profileData !== 'object') {
        console.warn(`[${transactionId}] Invalid profile data:`, profileData);
