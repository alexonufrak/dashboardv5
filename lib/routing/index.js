...
export const ROUTES = {
  ...
  PROGRAM: {
    INDEX: '/program',
    DETAIL: (programId) => `/program-new/${encodeURIComponent(programId)}`,
    MILESTONES: (programId) => `/program-new/${encodeURIComponent(programId)}/milestones`,
    TEAM: (programId) => `/program-new/${encodeURIComponent(programId)}/team`,
    BOUNTIES: (programId) => `/program-new/${encodeURIComponent(programId)}/bounties`,
    // Human-friendly URLs with slugs
    SLUG: (programId, slug) => `/program-new/${encodeURIComponent(programId)}/${encodeURIComponent(slug || 'details')}`,
    SLUG_MILESTONES: (programId, slug) => `/program-new/${encodeURIComponent(programId)}/${encodeURIComponent(slug || 'details')}/milestones`,
    SLUG_TEAM: (programId, slug) => `/program-new/${encodeURIComponent(programId)}/${encodeURIComponent(slug || 'details')}/team`,
    SLUG_BOUNTIES: (programId, slug) => `/program-new/${encodeURIComponent(programId)}/${encodeURIComponent(slug || 'details')}/bounties`
  },
...
export function isProgramRoute(router) {
  return router.pathname.startsWith('/program/') || router.pathname.startsWith('/program-new/');
}

/**
 * Check if the current route is a programs list route
 * @param {object} router - Next.js router object
 * @returns {boolean} True if the current route is the programs list page
 */
export function isProgramsListRoute(router) {
  return router.pathname === '/dashboard/programs';
}
...
export function isProgramSection(router, section) {
  return router.pathname === `/program/[programId]/${section}` || 
         router.pathname === `/program-new/[programId]/${section}`;
}