...
export const ROUTES = {
  ...
  PROGRAMS_APPLY: (cohortId, initiative) => {
    const base = `/dashboard/programs/apply/${encodeURIComponent(cohortId)}`;
    return initiative ? `${base}?initiative=${encodeURIComponent(initiative)}` : base;
  },
...
export function navigateToProgramApplication(router, cohortId, initiativeName, options = {}) {
  ...
  const url = ROUTES.PROGRAMS_APPLY(cohortId, initiativeName);
