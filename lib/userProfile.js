  ...
  try {
    ...
    try {
      ...
      if (isMinimal) {
        ...
        const minimalResults = await Promise.all([
          minimalPromises.participationPromise,
          minimalPromises.suggestedInstitutionPromise
        ]);
        
        // Assign results
        participationRecords = minimalResults[0] || [];
        suggestedInstitution = minimalResults[1] || null;
        ...
        // Always fetch cohorts - they're needed for the dashboard
        if (institutionId) {
          cohorts = await getCohortsByInstitution(institutionId);
          console.log(`Minimal mode: Fetched ${cohorts.length} cohorts for institution`);
        } else {
          cohorts = [];
        }
      } else {
