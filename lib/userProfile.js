...
export async function getCompleteUserProfile(auth0User) {
  try {
    ...
    // Safely handle these variables in case they weren't properly defined in the try/catch block
    const safeParticipationRecords = participationRecords || [];
    const safeTeamRecords = teamRecords || [];
    
    // Process participation data
    const hasActiveParticipation = Array.isArray(safeParticipationRecords) && safeParticipationRecords.length > 0;
    
    // Determine if any participation is a team initiative
    const hasTeamParticipation = Array.isArray(safeParticipationRecords) && 
                               safeParticipationRecords.some(p => p && p.isTeamParticipation) || false;
    
    // Extract active initiatives from participation records
    const activeInitiatives = Array.isArray(safeParticipationRecords) 
      ? safeParticipationRecords
    ...
    // Format team data
    const formattedTeams = Array.isArray(safeTeamRecords) 
      ? safeTeamRecords 
      : (safeTeamRecords ? [safeTeamRecords] : []);
    ...
    const completeProfile = {
      ...
      participations: safeParticipationRecords,
      teams: formattedTeams,
      relevantCohortIds: Array.isArray(safeParticipationRecords) && safeParticipationRecords.length > 0
        ? [...new Set(
            safeParticipationRecords
