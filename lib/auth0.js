...
export const auth0 = new Auth0Client({
  ...
  session: {
    ...
    // Override cookie settings to fix authentication issues
    // Force sameSite=none to ensure cookies are sent with all request types
    cookie: {
      httpOnly: true,
      secure: true, // Essential for secure sessions across all request types
      sameSite: 'none', // Explicitly set sameSite to none for cross-origin compatibility
      domain: process.env.NODE_ENV === 'production' ? '.xfoundry.org' : undefined,
      path: '/',
      transient: false // Make cookies persistent instead of session-based
    },
    name: 'appSession',
    storeIDToken: true,
    autoSave: true,
