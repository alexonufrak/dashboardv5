...
export const getManagementClient = () => {
  try {
    // Use separate environment variables specifically for Management API
    // Fall back to regular Auth0 variables only if Management-specific ones aren't defined
    const mgmtDomain = process.env.AUTH0_MGMT_API_DOMAIN || ENV.DOMAIN;
    const mgmtClientId = process.env.AUTH0_MGMT_API_CLIENT_ID || ENV.CLIENT_ID;
    const mgmtClientSecret = process.env.AUTH0_MGMT_API_CLIENT_SECRET || ENV.CLIENT_SECRET;
    
    console.log(`Creating Management API client for domain: ${mgmtDomain}`);
    
    // Create the client using dedicated Management API credentials
