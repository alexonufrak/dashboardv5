...
export const auth0 = new Auth0Client({
  ...
  session: {
    ...
    cookie: {
      ...
      sameSite: 'none', // Allow cross-site requests (more permissive than lax)
      transient: false, // Not transient - persist after browser close
      domain: process.env.NODE_ENV === 'production' ? '.xfoundry.org' : undefined // Match domain for production
    },
    name: 'appSession', // Explicitly name the cookie for consistency
    storeIDToken: true, // Store the ID token in the session
    autoSave: true, // Always save the session after changes
    rolling: true // Refresh the session with each request
  },
  
  // Enable both cookie-based and header-based authentication for APIs
  authRequired: false, // Only verify authentication when requested (don't auto-reject)
  routes: {
