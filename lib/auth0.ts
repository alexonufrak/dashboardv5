...
/**
 * Get an Auth0 Management API client
 * 
...
export async function getAuth0ManagementClient(): Promise<ManagementClient> {
  return getDirectManagementClient();
}
 ...
 */
async function getDirectAuth0Token(): Promise<string | null> {
  try {
 ...
 * @returns {Promise<ManagementClient>} Auth0 Management API client
 */
async function getDirectManagementClient(): Promise<ManagementClient> {
  try {
    // Get the token
...
interface AirtableUser {
  ...
  'First Name'?: string;
  'Last Name'?: string;
  'Institution (from Education)'?: string[];
  'Graduation Year (from Education)'?: string[];
  'Degree Type (from Education)'?: string[];
  'Major (from Education)'?: string[];
  Education?: string[];
  [key: string]: any;
}
...
export default {
   ...
   */
  getUser: async (params: { id: string }): Promise<Auth0User> => {
   ...
   */
  getUserByEmail: async (email: string): Promise<Auth0User | null> => {
    try {
      const client = await getDirectManagementClient();
  ...
  checkUserExistsByEmail: async (email: string): Promise<boolean> => {
    try {
      ...
      try {
        ...
        console.error('Error calling Auth0 API:', apiError);
   ...
   */
  getSignupMetadata: async (email: string, airtableUser: AirtableUser | null = null): Promise<SignupMetadata> => {
    try {
   ...
   */
  updateUserMetadata: async (params: { id: string }, metadata: Record<string, any>): Promise<any> => {
    try {
   ...
   */
  prepareAirtableMetadata: async (email: string, airtableData: AirtableUser): Promise<Record<string, any>> => {
    try {
   ...
   */
  updateUserFromAirtable: async (email: string, airtableData: AirtableUser): Promise<any | null> => {
    try {
      const client = await getDirectManagementClient();
      ...
      if (auth0User) {
        ...
        try {
          ...
          return client.users.update({ id: auth0User.user_id as string }, {
