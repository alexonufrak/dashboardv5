...
export async function getParticipationRecords(contactId) {
  try {
    ...
    // Use a formula that directly references the Contacts field which links to contacts
    // Only include active participation records
    const formula = `AND(
      OR(
        SEARCH("${safeContactId}", {Contacts}),
        SEARCH("${safeContactId}", {contactId})
      ), 
      OR({Status}="Active", {Status}="")
    )`;
    
    console.log(`Using participation lookup formula: ${formula.replace(/\s+/g, ' ')}`);
    
    // Query the participation table directly for records matching this contact
    ...
    for (const record of participationRecords) {
      try {
        ...
        // Process each cohort for this participation
        for (const cohortId of cohortIds) {
          try {
            ...
            // Create an enhanced record with all needed information
    ...
    console.log(`Successfully processed ${enhancedRecords.length} participation records with details`);
