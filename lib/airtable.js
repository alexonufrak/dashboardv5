...
export async function getUserTeams(contactId) {
  try {
    ...
    // First, find all membership records for this user
    const membershipRecords = await membersTable.select({
      filterByFormula: `{Contact}="${contactId}"`,
      fields: ["Team", "Status", "Role", "Points"]
    }).firstPage();
    
    console.log(`Found ${membershipRecords.length} membership records for contact ${contactId}`);
    
    // If no memberships found, return null
    if (membershipRecords.length === 0) {
      return null;
    }
    
    // Get the user's active teams
    const teamIds = membershipRecords
      .filter(record => record.fields.Status === "Active")
      .map(record => record.fields.Team?.[0])
      .filter(Boolean);
    
    // If user has no active teams, return null
    if (teamIds.length === 0) {
      return null;
    }
    
    console.log(`User has ${teamIds.length} active teams with IDs:`, teamIds);
    
    // Get the first team (we'll assume a user is only in one active team at a time)
    ...
    console.log(`Found team: ${team.fields.Name}`);
    
    // Get all members of this team
    const teamMembers = await membersTable.select({
      filterByFormula: `{Team}="${teamId}"`,
      fields: ["Contact", "Status", "Role", "Points"]
    }).firstPage();
    
    // Get contact IDs for all members
    const memberContactIds = teamMembers.map(member => member.fields.Contact?.[0]).filter(Boolean);
