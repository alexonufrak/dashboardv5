      ...
      if (cohort.Initiative && cohort.Initiative.length > 0) {
        ...
        if (initiativeDetails[initiativeId]) {
          ...
          result.initiativeDetails = {
            ...details,
            // Ensure Participation Type is available, defaulting to "Individual"
            "Participation Type": (participationType && typeof participationType === 'string') 
              ? participationType 
              : "Individual"
          };
          
          // Add direct access to participation type to make it easier to check
          result.participationType = result.initiativeDetails["Participation Type"];
          
          console.log(`Initiative ${initiativeId} participation type: ${result.participationType}`);
        }
      }
