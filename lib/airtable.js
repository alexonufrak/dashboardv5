...
export async function updateUserProfile(contactId, data) {
  try {
    ...
    if (data.educationId) {
      console.log(`Updating education record ${data.educationId} with academic information`);
      ...
      // Handle Graduation Year field - ensure it's a number
      if (data.GraduationYear) {
        // Remove any non-digit characters and convert to number
        const graduationYear = parseInt(String(data.GraduationYear).replace(/\D/g, ''), 10);
        
        // Only add if it's a valid number
        if (!isNaN(graduationYear)) {
          educationData["Graduation Year"] = graduationYear;
        } else {
          console.log("Warning: Invalid graduation year format", data.GraduationYear);
        }
      }
      ...
      await educationTable.update(data.educationId, educationData);
    } 
    // If we don't have an education record but have institution data, create a new education record
    else if (data.InstitutionId) {
      console.log("Creating new education record with institution:", data.InstitutionId);
      
      // Create basic education data
      const educationData = {
        "Contact": [contactId], // Link to the contact
        "Institution": [data.InstitutionId], // Link to the institution
        "Degree Type": data.DegreeType,
      };
      ...
      console.log("No institution data provided to create education record");
    }
