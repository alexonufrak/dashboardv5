...
export async function updateUserProfile(contactId, data) {
  try {
    ...
    // If we have an education record ID, update the education record too
    if (data.educationId) {
      ...
      // so we need to provide it as an array if it's selected
      if (data.Major) {
      ...
      console.log("No education record ID provided for update");
    }
