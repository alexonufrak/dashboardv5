...
export async function checkInitiativeConflicts(contactId, initiativeName) {
  try {
    ...
    // Check if requested initiative has team participation
    const isRequestedTeamParticipation = 
      requestedInitiativeParticipationType.toLowerCase() === "team" || 
      requestedInitiativeParticipationType.toLowerCase().includes("team");
    
    console.log(`Is requested initiative "${initiativeName}" a team initiative? ${isRequestedTeamParticipation}`);
    ...
    for (const id in initiativeInfo) {
      ...
      // Check for team participation type conflicts
      const isCurrentTeamParticipation = 
        currentParticipationType === "team" || 
        currentParticipationType.includes("team");
      
      console.log(`Current initiative "${initiative.name}" is team participation? ${isCurrentTeamParticipation}`);
