...
export async function checkInitiativeConflicts(contactId, initiativeName) {
  try {
    ...
    if (conflictingInitiative) {
      ...
      // Find the participation record with the conflicting initiative
      const conflictingParticipation = participations.find(
        p => p.get('Initiative')?.length > 0 && 
             initiatives.find(i => i.id === p.get('Initiative')[0])?.get('Name') === conflictingInitiative
      );
      ...
      if (conflictingParticipation) {
        const teamRecordId = conflictingParticipation.get('Team');
        
        if (teamRecordId && Array.isArray(teamRecordId) && teamRecordId.length > 0) {
          teamId = teamRecordId[0];
          
          // Try to get team name from the expanded record or lookup field
          if (conflictingParticipation._rawJson && 
              conflictingParticipation._rawJson.fields) {
            // Try Team Name lookup field first
            if (conflictingParticipation._rawJson.fields['Team Name']) {
              teamName = conflictingParticipation._rawJson.fields['Team Name'];
            }
            // Fall back to Team Lookup if available
            else if (conflictingParticipation._rawJson.fields['Team Lookup'] && 
                Array.isArray(conflictingParticipation._rawJson.fields['Team Lookup']) && 
                conflictingParticipation._rawJson.fields['Team Lookup'].length > 0) {
              teamName = conflictingParticipation._rawJson.fields['Team Lookup'][0];
            }
          }
        }
      }
