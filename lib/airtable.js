...
/**
 * Fetches data from Airtable with throttling and 429 error handling.
 * Server-side caching has been removed.
 * @param {string} cacheKey - Identifier for logging purposes (no longer used for caching).
 * @param {Function} fetchFn - Function to execute to fetch data.
 * @param {number} ttl - TTL (ignored, kept for signature compatibility).
...
export const getCachedOrFetch = async (cacheKey, fetchFn, ttl = 300, retryCount = 0) => {
  ...
  // Fetch fresh data (no server caching)
  console.log(`Fetching data (no server cache) for key: ${cacheKey}`)
  ...
  try {
    ...
    if (error.statusCode === 429) {
      ...
      // Pass a dummy cacheKey as it's no longer used for caching but might be expected by recursive call structure
      return getCachedOrFetch(`retry-${cacheKey}`, fetchFn, ttl, retryCount + 1)
    }
...
/**
 * Clears cache entries by type and optional ID (No-op after cache removal)
 * @param {string} type - Cache type (ignored)
 * @param {string} id - Optional entity ID (ignored)
 * @returns {number} Always returns 0
 */
export const clearCacheByType = (type, id = null) => {
  console.log(`Server cache clear requested for type: ${type}${id ? ` ID: ${id}` : ''} (No-op)`);
  return 0; // Return 0 as no entries are cleared
}

/**
 * Legacy function for backward compatibility (No-op after cache removal)
 * @param {string} keyPattern - Pattern to match (ignored)
 * @param {Object} details - Additional details (ignored)
 * @returns {number} Always returns 0
 */
export const clearCacheByPattern = (keyPattern, details = null) => {
  console.log(`[DEPRECATED] Server cache clear requested by pattern: ${keyPattern} (No-op)`);
  return 0; // Return 0 as no entries are cleared
}

/**
 * Batch fetch records from Airtable. Server-side caching removed.
 * @param {string} tableId - Airtable table ID
 * @param {Object} options - Query options
 * @param {string} cacheTypeOverride - Ignored
 * @param {string} entityIdOverride - Ignored
 * @param {boolean} forceRefresh - Ignored
...
export const batchFetchRecords = async (tableId, options, cacheTypeOverride = null, entityIdOverride = null, forceRefresh = false) => {
  // Apply throttling before fetching
  await throttleRequests();

  console.log(`Fetching batch records (no server cache) from table ${tableId}`);

  try {
    const table = base(tableId);
    // Use .all() for automatic pagination handling by the Airtable SDK
    const records = await table.select(options).all();
    return records;
  } catch (error) {
    console.error(`Error batch fetching records from ${tableId}:`, error);
    // Handle rate limit errors specifically if needed, otherwise re-throw
    if (error.statusCode === 429) {
       console.error('Airtable rate limit hit during batch fetch.');
       // Consider adding retry logic here similar to getCachedOrFetch if needed
    }
...
export async function getParticipationRecords(contactId) {
  try {
    ...
    // Try formula approach
    let participationRecords = [];

    try {
      // Use formula 1 to find participation records
      console.log(`Using participation lookup formula: ${formula1}`);
      ...
      if (records && records.length > 0) {
        ...
        console.log(`Found ${records.length} participation records with formula`);
      }
    } catch (error) {
    ...
    const processRecordPromises = participationRecords.map(async (record) => {
      try {
        ...
        if (cohortIds.length === 0) {
          console.log(`Participation record ${record.id} has no cohorts - will attempt to use linked data`);
          console.log(`Record fields available: ${Object.keys(record.fields).join(', ')}`);

          // Instead of skipping this record entirely, try to get info from related records
          // First check if there's an initiative directly linked
          if (record.fields.Initiative && record.fields.Initiative.length > 0) {
            try {
              const initiativeId = record.fields.Initiative[0];
              console.log(`Found initiative ID ${initiativeId} - attempting to load initiative details`);

              const initiative = await initiativesTable.find(initiativeId);
              console.log(`Successfully loaded initiative: ${initiative.fields.Name || 'Unnamed'}`);

              // Create a basic record with the minimal information available
              return {
                id: record.id,
                ...
                initiative: {
                  ...
                  isTeamBased: (initiative.fields["Participation Type"] || "").toLowerCase().includes("team")
                },
                team: record.fields.Team && record.fields.Team.length > 0 ? { id: record.fields.Team[0] } : null,
                isTeamParticipation: (initiative.fields["Participation Type"] || "").toLowerCase().includes("team"),
                recordFields: record.fields
              };
            } catch (initiativeError) {
              console.error(`Error processing initiative for cohortless record ${record.id}:`, initiativeError);
              console.error(`Initiative error details:`, initiativeError.message || initiativeError);
            }
          } else {
            console.log(`No Initiative field found or it's empty in record ${record.id}`);
          }

          // Additional fallback for records with a program field but no initiative
          if (record.fields.Program && record.fields.Program.length > 0) {
            try {
              const programId = record.fields.Program[0];
              console.log(`Found program ID ${programId} - attempting to use program as fallback`);

              const program = await programsTable.find(programId);
              console.log(`Successfully loaded program: ${program.fields.Name || 'Unnamed'}`);

              // Create a basic record with program information
              return {
                id: record.id,
                status: record.fields.Status || "Active",
                capacity: record.fields.Capacity || "Participant",
                cohort: {
                  id: null,
                  name: program.fields.Name ? `${program.fields.Name} Program` : "Program",
                  shortName: program.fields.Name || "",
                  status: "Active",
                  isCurrent: true,
                  programId: programId
                },
                initiative: {
                  id: programId, // Use program ID as initiative ID for compatibility
                  name: program.fields.Name || "Untitled Program",
              ...
              console.error(`Error processing program for cohortless record ${record.id}:`, programError.message || programError);
            }
          } else {
            console.log(`No Program field found or it's empty in record ${record.id}`);
          }

          console.log(`Unable to process participation record ${record.id} - no cohorts, initiatives, or programs found`);

          // Create an ultra-basic fallback record with minimal information
          if (record.fields.Status === "Active") {
            console.log(`Creating minimal fallback record for active participation ${record.id}`);
            return {
              id: record.id,
              status: "Active",
              capacity: record.fields.Capacity || "Participant",
    ...
    // If we don't have any active records but we have participation records in the database,
    // create a minimal fallback record to prevent dashboard errors
    if (activeRecords.length === 0 && participationRecords.length > 0) {
      console.log(`No active records found but ${participationRecords.length} records exist - creating fallback record`);

      // Find the most likely record to use as fallback
      const fallbackRecord = participationRecords[0];
      console.log(`Using fallback record ${fallbackRecord.id} with fields: ${Object.keys(fallbackRecord.fields).join(', ')}`);

      // Create a basic fallback record
      return [{
        id: fallbackRecord.id,
        status: "Active", // Force active status for UI
        capacity: fallbackRecord.fields.Capacity || "Participant",
        cohort: {
          id: null,
          name: "Program Participation",
          shortName: "",
        ...
        team: fallbackRecord.fields.Team && fallbackRecord.fields.Team.length > 0
          ? { id: fallbackRecord.fields.Team[0] }
          : null,
        isTeamParticipation: false,
        recordFields: fallbackRecord.fields,
...
/**
 * Update user profile in Airtable using PATCH semantics (only update specified fields)
 *
 * Note: Airtable's JavaScript SDK .update() method uses PATCH semantics by default,
 * which means it only updates the fields provided and preserves existing fields.
 * This is the behavior we want for profile updates to avoid data loss.
 *
 * @param {string} contactId - Airtable contact ID
...
export async function updateUserProfile(contactId, data) {
  try {
    ...
    // Use Airtable's update method which performs a partial update (PATCH semantics)
    ...
    if (educationId) {
      ...
      if (Object.keys(educationData).length > 0) {
        ...
        // Use Airtable's update method which preserves fields not included in the update
