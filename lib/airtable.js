...
export async function updateUserProfile(contactId, data) {
  try {
    ...
    if (educationId) {
      ...
      if (data.Major) {
        // Make sure Major is formatted correctly as a record ID for linking
        if (typeof data.Major === 'string' && data.Major.trim() !== '') {
          // Format as array with single ID - no need for excessive logging
          educationData["Major"] = [data.Major]; 
        } else if (data.Major) {
          console.warn("Major field is not a valid string or is empty:", data.Major);
        }
      }
...
export async function getAllPrograms() {
  try {
    ...
    // Filter out any records that don't have a valid Airtable ID
    const validRecords = records.filter(record => {
      const hasValidId = record.id && record.id.startsWith('rec');
      if (!hasValidId) {
        console.warn(`Filtered out program with invalid ID: ${record.id}`);
      }
      return hasValidId;
    });
