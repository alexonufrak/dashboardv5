...
/**
 * Clears cache for a specific key pattern
 * @param {string} keyPattern - Part of the key to match for clearing
 */
export const clearCacheByPattern = (keyPattern, details = null) => {
  console.log(`Clearing cache entries containing pattern: ${keyPattern}`);
  let clearedCount = 0;
  let clearedKeys = [];
  ...
  if (details && details.teamId) {
    ...
    // Look for cache entries that match both the table ID and team ID
  ...
  console.log(`Cleared ${clearedCount} cache entries matching: ${keyPattern}`);
...
/**
 * Batch fetch records from Airtable
 * @param {string} tableId - Airtable table ID
 * @param {Object} options - Query options
 * @param {boolean} forceRefresh - Skip cache and fetch fresh data
 * @returns {Promise<Array>} Array of records
 */
export const batchFetchRecords = async (tableId, options, forceRefresh = false) => {
  const cacheKey = `batch_${tableId}_${JSON.stringify(options)}`;
