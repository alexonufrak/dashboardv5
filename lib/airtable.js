    ...
    const records = await contactsTable
      .select({
        ...
        fields: [
          ...
          "Graduation Year (from Education)"
        ],
      })
      .firstPage()
...
export async function getParticipationRecords(contactId) {
  try {
    ...
    console.log(`Successfully processed ${flattenedRecords.length} participation records with details`);
    ...
    if (teamIds.length <= 10) {
      ...
      return records.map(record => ({
        ...
        institution: record.fields.Institution ? {
          id: Array.isArray(record.fields.Institution) ? record.fields.Institution[0] : record.fields.Institution,
          name: record.fields["Institution Name"]
        } : null,
        displayMembers: record.fields["Member Names"] || [],
...
export async function checkInitiativeConflicts(contactId, initiativeName) {
  try {
    ...
    if (conflictingInitiative) {
      ...
      // IMPORTANT: We should ONLY use participation records for conflict checking
      // We're intentionally removing the team member lookup as it can cause false positives
      // If there's no team ID from participation records, we won't look for one
      if (!teamId) {
        console.log("No team ID found in participation records - using generic values");
        
        // Do NOT look up team membership as it may be for non-conflicting programs
        // Instead, use generic values:
        teamId = "unknown";
