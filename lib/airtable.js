...
export async function getUserTeams(contactId) {
  try {
    ...
    console.log(`Looking for member records with Contact="${contactId}"`);
    ...
    if (teamIds.length === 0) {
      console.log("User has no active teams");
      return null;
    }
    ...
    console.log(`Getting all members of team ${teamId}`);
    ...
    const memberContactIds = teamMembers
      .filter(member => member.fields.Status === "Active") // Only get active members
