...
export function handleAirtableError(error, operation, context = {}) {
  ...
  // Check for schema errors (422 Unprocessable Entity)
  else if (error.statusCode === 422 || 
          (error.message && (
            error.message.includes('Unknown field name') || 
            error.message.includes('UNKNOWN_FIELD_NAME')
          ))) {
    userMessage = 'Database schema error: The structure may have changed. Please contact support.';
    // Log the specific field issue
