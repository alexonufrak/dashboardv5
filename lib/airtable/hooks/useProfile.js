 ...
 * @param {boolean} options.refresh Force a refresh of the profile data
 * @returns {Object} Query result with user profile data
 */
export function useProfileData(options = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const url = refresh 
        ? '/api/user/profile-v2?refresh=true' 
        : '/api/user/profile-v2';
...
export function useUpdateProfile() {
  ...
  const mutation = useMutation({
    mutationFn: async (updatedData) => {
      ...
      // If this profile has education data, ensure it's properly merged
      if (profileData && profileData.education && profileData.education.length > 0) {
        const educationId = profileData.education[0];
        
        // Try to fetch education data if not already included
        if (!profileData.institutionName && !profileData.educationData) {
          try {
            ...
            if (educationResponse.ok) {
              ...
              if (educationData && educationData.education) {
                console.log('Merging education data with profile');
                return {
                  ...profileData,
                  educationData: educationData.education,
                  educationId: educationData.education.id,
                  institutionName: educationData.education.institutionName,
                  institution: educationData.education.institution,
                  degreeType: educationData.education.degreeType,
                  graduationYear: educationData.education.graduationYear,
                  graduationSemester: educationData.education.graduationSemester,
                  major: educationData.education.major,
                  majorName: educationData.education.majorName
                };
              }
            }
          } catch (error) {
            console.error('Error fetching education data:', error);
            // Continue with the profile data even if education fetch fails
          }
        }
      }
