...
async function findUserViaLinkedRecords(email) {
  ...
  // Import specific table functions to avoid requiring the full tables module
  const { 
    getApplicationsTable,
    getMembersTable,
    getAttendanceTable 
  } = await import('../tables/index.js');
...
async function fetchApplicationsByContactId(contactId) {
  ...
  try {
    // Import the applications table directly with exact path
    const { getApplicationsTable } = await import('../tables/index.js');
    const applicationsTable = getApplicationsTable();
    ...
    console.log(`Found ${applications.length} applications for contact ID ${contactId}`);
