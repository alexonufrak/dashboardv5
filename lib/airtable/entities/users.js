...
export async function getCompleteProfile(auth0User, options = {}) {
  try {
    ...
    // If we don't have a profile, try to find via linked record paths
    ...
    if (!options.skipApplications && profile.contactId) {
      try {
        ...
        enhancedProfile.hasApplications = applications.length > 0;
      } catch (appErr) {
...
async function findUserViaLinkedRecords(email) {
  ...
  // Import the tables module to get table functions
  // Avoid using getTable which doesn't exist
  const { 
  ...
  try {
    ...
    if (applications && applications.length > 0 && applications[0].fields.Contact?.[0]) {
      const contactId = applications[0].fields.Contact[0];
      console.log(`Found user via Applications table, Contact ID: ${contactId}`);
      return getUserByRecordId(contactId);
    }
  } catch (err) {
  ...
  try {
    ...
    if (members && members.length > 0 && members[0].fields.Contact?.[0]) {
      const contactId = members[0].fields.Contact[0];
  ...
  try {
    ...
    if (attendanceRecords && attendanceRecords.length > 0 && attendanceRecords[0].fields.Contacts?.[0]) {
      const contactId = attendanceRecords[0].fields.Contacts[0];
...
async function fetchApplicationsByContactId(contactId) {
  ...
  try {
    // Import the applications table directly
    const { getApplicationsTable } = await import('../tables');
    const applicationsTable = getApplicationsTable();
    ...
    throw handleAirtableError(error, 'fetching applications by contact ID', { contactId });
  }
}
