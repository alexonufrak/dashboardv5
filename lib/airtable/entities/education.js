...
export async function updateEducation(data) {
  try {
    ...
    if (educationId) {
      ...
      console.log(`Updating existing education record: ${educationId}`);
      educationRecord = await executeQuery(() => 
        educationTable.update(educationId, recordData)
      );
    } else {
      // Create new record
      console.log(`Creating new education record for contact: ${contactId}`);
      educationRecord = await executeQuery(() => 
        educationTable.create(recordData)
      );
      
      // Get current education records first to avoid overwriting
      try {
        const contact = await executeQuery(() => contactsTable.find(contactId));
        const existingEducation = contact.fields.Education || [];
        
        // Only update if we need to add this new record
        if (!existingEducation.includes(educationRecord.id)) {
          console.log(`Adding new education record ${educationRecord.id} to contact's existing records: ${existingEducation}`);
          
          await executeQuery(() => 
            contactsTable.update(contactId, {
              "Education": [...existingEducation, educationRecord.id]
            })
          );
        }
      } catch (err) {
        console.error(`Error updating contact with new education record:`, err);
        // We won't fail the whole operation since the education record was created
      }
    }
