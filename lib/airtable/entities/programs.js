...
/**
 * Fetches all programs/majors without caching
 * @returns {Promise<Array>} Array of program objects
 */
export async function fetchAllPrograms() {
  try {
    ...
    console.log(`Found ${records.length} programs`);
    ...
    throw handleAirtableError(error, 'fetching all programs');
  }
}

/**
 * Gets all programs/majors with caching
 * @param {Object} options Cache options
 * @returns {Promise<Array>} Array of program objects
 */
export async function getAllPrograms(options = {}) {
  const cacheKey = createCacheKey(CACHE_TYPES.PROGRAMS, 'all_programs');
