...
export async function getCompleteUserProfile(auth0User, options = {}) {
  ...
  try {
    ...
    // First try to fetch profile by email, and fallback to Auth0 ID if that fails
    let airtableProfile = await getUserByEmail(auth0User.email);
    
    // If we can't find by email, try with Auth0 ID
    if (!airtableProfile) {
      console.log("Profile not found by email, trying Auth0 ID");
      airtableProfile = await getUserByAuth0Id(auth0User.sub);
    }
