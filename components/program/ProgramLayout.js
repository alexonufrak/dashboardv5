...
const ProgramLayout = ({ children, programId, activeTab = 'overview' }) => {
  ...
  const { getActiveProgramData, getAllProgramInitiatives } = useDashboard()
  ...
  const handleTabChange = (value) => {
    const basePath = `/program/${programId}`
    ...
    switch (value) {
      case 'overview':
        ...
        break
      case 'bounties':
        router.push(`${basePath}/bounties`)
        break
      case 'milestones':
        router.push(`${basePath}/milestones`)
        break
      case 'team':
