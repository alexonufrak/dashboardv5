 ...
 */
export default function ProgramDashboard({ programId: propProgramId }) {
  const router = useRouter();
  const { programId: routerProgramId } = router.query;
  const programId = propProgramId || routerProgramId;
  const { user } = useUser();
  ...
  // Filter participation for this program
  const participation = useMemo(() => {
    if (!participationData || !participationData.participation || !programId) {
      return null;
    }
    return participationData.participation.find(p => p.initiative?.id === programId);
  }, [participationData, programId]);
